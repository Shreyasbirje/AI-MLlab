...
def NetValue(W):
    ...
    Net = []
    for i in X:
        # formula used to calculate net value wrt imputs x1,x2
        NetV = FinalW1*i[0]+FinalW2*i[1]+FinalBias
        Net.append(NetV)
...
def ANDwithHebb_LR():
    ...
    while count < 4:  # for learning and updating weights
        for i in X:
            temp = []
            # formula to update weights Weight(new)=Weight(old)+x*y
            weight1 += i[0] * Y[count]
            weight2 += i[0]*Y[count]
    ...
    for i in ResultFromNet:
        if i > 0:
            ...
            Prediction.append(-1)
    print("Required >>>{}\nPredicted From Hebbian Learning Rule >>>{}".format(
        Y, Prediction))
